...
class CryptoApp(App):
    def build(self):
        ...
        # Buttons
        btn_layout = BoxLayout(spacing=5, size_hint_y=None, height=40)
        ...
        self.layout.add_widget(Label(
            text='[b]Enhanced Caesar Cipher[/b]', 
            markup=True,
            size_hint_y=None,
            height=40
        ))
    ...
    def encrypt(self, instance):
        ...
        if not password or not message:
            self.show_error("Password and message are required!")
            return
            
        try:
            cipher = EnhancedCaesarCipher(password)
            encrypted = cipher.encrypt(message)
            self.output.text = encrypted
        except SecurityException as se:
            self.show_error(f"Security error: {str(se)}")
    ...
    def decrypt(self, instance):
        ...
        if not password or not message:
            self.show_error("Password and encrypted data are required!")
            return
